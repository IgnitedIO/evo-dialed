...
async function db_verifyUser(username, password, cb) {
  ...
  try {
    ...
    const db_resp = await knex('Users_Auth').join('Users', 'Users_Auth.user_id', 'Users.id').where('Users.email', email).select('Users_Auth.salt AS salt', 'Users_Auth.hash AS hash', 'Users_Auth.user_id AS uid', 'Users.user_typ AS user_typ').limit(1).catch(db_err => { if (db_err) err_code = db_err.code; });
    ...
    crypto.pbkdf2(password, db_resp[0].salt, HASH_ITERATIONS, 32, 'sha256', async function(err, hashed_input) {
      ...
      // Confirmed sign-in
      return cb(null, { 'id': db_resp[0].uid, 'username': email, 'user_typ': db_resp[0].user_typ });
    });
  } catch (err) {
...
nativePass.serializeUser(function(user, cb) {
  process.nextTick(function() {
    cb(null, { id: user.id, username: user.username, user_typ: user.user_typ });
  });
});
